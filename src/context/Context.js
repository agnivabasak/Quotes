...
const Reducer = (state,action) =>{
    ...
    switch(action.type)
    {
        ...
        case "add_bookmark" : {
            ...
            let state2 =state;
            let quoteInfo = {quote : action.payload.quote,author : action.payload.author,id :action.payload.id}
        ...
        case "delete_bookmark" : {
            let state2 =state;
            state2.Bookmarks = state.Bookmarks.filter((item)=>item.id!==action.payload.id);
        ...
        case "change_status" : {
            ...
            state2.Bookmarks = state.Bookmarks.map((item)=>{
                ...
                {
                    return {id : item.id,author : item.author,quote : item.quote,bookmarked : !item.bookmarked}
                }
                return item;
            })
            return state2;
...
export const {Context,Provider} = createContext(Reducer,{GetQuotesList,GetBookmarks,AddToBookmark,DeleteFromBookmark,ChangeBookmarkStatus},{QuotesList : [],Bookmarks:[],QuoteOfTheDay : ""});
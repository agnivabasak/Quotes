...
export default function Carousel ({QuotesList,opacities}){
    return <View style={styles.carousel}>
        {QuotesList.map((item,index)=>{
          ...
          return index!=0? <CarouselCard key={index} opacity = {opacities[index]} quote = {item.quote} author = {item.author} id= {item.id} beg ={false} bookmarked={item.bookmarked}/> : <CarouselCard quote = {item.quote} opacity={opacities[index]} author = {item.author} id= {item.id} beg={true} bookmarked={item.bookmarked}/>;
        })}
    </View>
}
